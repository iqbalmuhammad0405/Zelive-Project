  ...
  template: `
    <ion-header class="ion-no-border">
      <ion-toolbar class="glass-toolbar">
        ...
        <ion-title class="brand-title">Edit Product</ion-title>
      </ion-toolbar>
    </ion-header>
    ...
    <ion-content class="navy-dashboard-bg">
      <div class="form-wrapper">
        <div class="form-container">
          ...
          <button class="submit-btn" (click)="processPublishProduct()" [disabled]="isSubmitting || isLoadingProduct">
...
export class SellerEditProductComponent implements OnInit {
  ...
  loadProduct() {
    ...
    this.apiService.get(`products/${this.productId}`).subscribe({
      next: (res: any) => {
        ...
        if (p.image_url) {
          this.previewUrl = p.image_url;
          this.fileName = 'Current Image';
          // Create a dummy file object to bypass validation if user doesn't pick a new one
          this.selectedFile = new File([''], 'existing-image', { type: 'image/jpeg' });
        }
  ...
  private updateProduct() {
    ...
    this.apiService.put(`products/${this.productId}`, this.newProduct).subscribe({
      next: (res: any) => {
        try {
          if (!res || !res.data || !res.data.id) {
            throw new Error('Invalid API response from create product');
          }
          const productId = res.data.id || this.productId;
          // If the file is 'existing-image', we don't need to re-upload
          if (this.selectedFile && this.selectedFile.name !== 'existing-image') {
